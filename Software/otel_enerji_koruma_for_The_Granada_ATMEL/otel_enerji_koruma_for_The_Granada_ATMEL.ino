...
{ 
  if (x == 0 && digitalRead(BEING) == HIGH){x = 1;old_time_1 = millis();Serial.println("Varlık Algılandı, Röle Açıldı");}
  else if ((y == 1) && (x == 1 && digitalRead(BEING) == LOW)) {now_time_1 = millis();if ((now_time_1 - old_time_1 == day_time) && (digitalRead(BEING) == LOW) ){x = 0;old_time_1 = millis();Serial.println("Röle Kapandı");}}
  else {}
}
// ***************************************** //
...
if (x == 3 && digitalRead(BEING) == HIGH){old_time_1 = millis();Serial.println("Varlık Algılandı");x = 1;}
if (x == 3 && digitalRead(BEING) != HIGH){now_time_1 = millis();if ((now_time_1 - old_time_1 == room_time) && (digitalRead(BEING) == LOW) ){x = 0;old_time_1 = millis();Serial.println("Varlık Algılanmadı, Röle Kapandı");}}
// ***************************************** //
if (x == 0) {digitalWrite(relay, LOW);}
// ***************************************** //
if (x == 1) {digitalWrite(relay, HIGH);}
// ***************************************** //
if (digitalRead(BEING) == HIGH){old_time_1 = millis(); y = 1;}
// ***************************************** //
}
