...
int zaman = 300; // 10 = 1 sn // person "0" olunca relay kapanma süresi
int dividing = 10; // zaman'ı bölen sayısı
int t = 500; // iki sensörden geçiş süresi
...
void electric_off(){
  ...
  Serial.println("⚡⚡ ELEKTRIK ⚡⚡ ⚡⚡ KESINTISI 'I2C' ⚡⚡"); //⚡⚡⚡⚡⚡⚡⚡
  }
}
...
void external_card() {
  if (digitalRead(EXTERNAL_RFID) == HIGH && millis() - time > debounce) {
    Serial.println("Card Entry");
    if (statePIN == HIGH)
      statePIN ;
    else
      statePIN = HIGH;
